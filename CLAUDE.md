...
| `src/shared/lib/` | Pure functions dipakai ketiga app: `qris.js` (crc16/qrisToDynamic), `shipping.js` (Haversine), `cart.js` (cart math + `cartStockItems`), `format.js`, `whatsapp.js` (message builder + `ORDER_STATUS_LABELS`), `csv.js` (hand-rolled parse/download, no library), `config.js` (baca env var `VITE_*`), `supabaseClient.js`, `products.js`/`promos.js`/`orders.js`/`settings.js` (mutation & RPC call functions ke Supabase) |
| `src/shared/hooks/` | `useProducts`, `usePromos`, `useOrders`, `useSettings` — data-fetching hooks dipakai lintas app |
