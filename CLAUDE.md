...
| `src/shared/lib/` | Pure functions dipakai ketiga app: `qris.js` (crc16/qrisToDynamic), `shipping.js` (Haversine), `cart.js` (cart math + `cartStockItems`), `format.js`, `whatsapp.js` (message builder + `ORDER_STATUS_LABELS`), `csv.js` (hand-rolled parse/download, no library), `production.js` (rekap produksi per tanggal, pure function di atas baris `orders`), `capacity.js` (hitung sisa slot, pure), `config.js` (baca env var `VITE_*`), `supabaseClient.js`, `products.js`/`promos.js`/`orders.js`/`settings.js`/`visits.js`/`closedDates.js` (mutation & RPC call functions ke Supabase) |
| `src/shared/hooks/` | `useProducts`, `usePromos`, `useOrders`, `useSettings`, `useDailyVisits`, `useCapacity`, `useFullDates`, `useClosedDates` — data-fetching hooks dipakai lintas app. Plus `useFavicon` (pasang `settings.favicon_url` ke `<link rel="icon">` saat runtime, dipanggil ketiga app) |
...
| `PROMO.md` | Bahan promo: alur pelanggan layar per layar, sisi pemilik toko, angle cerita, perbandingan paket, dan daftar hal yang jangan dijanjikan. Ditulis untuk video/deck jualan, bukan untuk coding. Kalau ada fitur baru yang kelihatan customer, sinkronkan ke sini juga. |
| `public/` | Favicon **default** (`favicon.ico`, `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, `android-chrome-*.png`, `site.webmanifest`) — Vite copy apa adanya ke root `dist/`, di-link dari ketiga `index.html` dengan path absolut (`/favicon.ico`, dst). Ini cuma fallback build-time: favicon per klien di-upload lewat Admin > Pengaturan (`settings.favicon_url`) dan menimpa link statis ini saat runtime lewat `useFavicon()`, jadi tidak perlu ganti isi folder ini per klien. |
...
**Tanggal yang gak bisa dipesan** (`useFullDates` → `get_full_dates()`, `supabase-setup.sql` §7): di-disable, biar customer gak pilih tanggal, masuk katalog, nemu semuanya penuh, lalu harus mundur. Kalau SEMUA tanggal kena, kalendernya diganti satu kalimat, bukan dibiarin jadi deretan chip mati.

Hook-nya balikin **Map tanggal → alasan**, bukan Set, dan chip-nya nulis beda: **"Libur"** (toko tutup tanggal itu, `closed_dates`) vs **"Penuh"** (kuotanya habis diambil orang lain). Jangan disamain — "Penuh" padahal toko libur bikin customer ngira kehabisan lalu nungguin tanggal itu dibuka lagi. Kalimat "semua tanggal" juga ikut bedanya. Libur dicek duluan dan menang kalau dua-duanya berlaku.
...
- **Hasil pencarian alamat dibias + diurutkan ke sekitar toko**, karena LocationIQ mengurutkan murni pakai relevansi teks dan gak tahu tokonya di mana (gejalanya: search "Taman Anggrek" dari Jakarta, hasil teratas Taman Anggrek Bandung). Dua lapis di `AddressPickerModal.jsx`, dua-duanya perlu:
  1. **`viewbox`** kotak radius `config.addressSearchRadiusKm` (env `VITE_ADDRESS_SEARCH_RADIUS_KM`, default 50 km) di sekitar `storeLat/storeLng`, dikirim ke LocationIQ. Ini yang menentukan *kandidat* mana yang balik, penting karena `limit=8` — kalau delapan-delapannya luar kota, sorting client-side gak bisa menyelamatkan apa-apa. **Sengaja tanpa `bounded=1`**, lihat "Kenapa begini".
  2. **Sort per pita jarak** (`DISTANCE_BAND_KM`, 10 km) pakai `haversineDistance()`, stabil. Bukan sort murni per jarak: dalam satu pita, urutan relevansi LocationIQ dipertahankan supaya "Mall Taman Anggrek" gak kalah sama gang kecil bernama "Anggrek" yang kebetulan 500 m lebih dekat. Yang digeser cuma yang beda pita, alias beda kota. Hasil tanpa koordinat valid ditaruh paling akhir (comparator-nya harus balik angka, bukan `NaN`, kalau `NaN` urutannya jadi acak).
  Tiap item juga nampilin "x.x km dari toko" — jarak lurus, sama sumbernya dengan yang dipakai `OngkirOptions`, jadi customer bisa langsung lihat mana yang masuk akal tanpa baca `display_name` yang panjang.
- **Ongkir**: `useShipping().calculate(lat, lng, weightGrams, orderValue)` cuma punya **dua** pemanggil, dua-duanya sekali-jalan, bukan `useEffect` yang watch perubahan alamat (itu justru bug yang mau dihindari, lihat "Kenapa begini" di bawah):
  1. handler save di `ProfileModal.jsx`, waktu customer baru mengisi/mengganti alamat;
  2. efek auto-cek di `CheckoutStep.jsx`, sekali saat step 3 di-mount, khusus kalau `state.profile.lat/lng` sudah ada (alamat dari kunjungan sebelumnya lewat `localStorage`) TAPI `shippingStatus` masih `'idle'`. Ini yang bikin customer balik gak perlu buka modal profil dan tekan Simpan cuma buat mancing cek ongkir. Guard-nya dua lapis: status harus `'idle'`, plus `useRef` yang mengunci per pasangan koordinat, jadi hasil `'unavailable'` atau perubahan cart tidak pernah memicu panggilan kedua. `goBack()` dari step 3 dispatch `CLEAR_SHIPPING` (bukan `SET_SHIPPING` null) supaya masuk lagi ke step 3 memicu cek ulang dengan berat cart yang baru. Hasilnya dispatch ke `CartContext` (`SET_SHIPPING_OPTIONS`/`SET_SHIPPING_STATIC`/`SET_SHIPPING_UNAVAILABLE`), ditampilkan `OngkirOptions.jsx` yang render di `CheckoutStep` (bukan di dalam modal profil). Koordinat resolved disimpan balik ke `state.profile.lat/lng` juga (bukan cuma state lokal `ProfileModal`), supaya map pin & hasil ongkir bisa dipulihkan kalau modal ditutup-buka lagi.
...
**Header admin tidak punya ikon default.** Slot ikon di `.admin-header .logo` cuma diisi `settings.logo_url` (logo ikon yang di-upload klien); kalau kosong, header cuma menampilkan nama toko, bukan emoji fallback — ikon bawaan yang jelas bukan milik klien lebih mengganggu daripada slot kosong. `.admin-header .logo` wajib `display:flex`: tanpa itu `<img>` (inline) dan `.logo-text` (block) jatuh atas-bawah dan bikin header ikut tinggi. `LoginScreen` pakai aturan yang sama tapi masih punya cadangan emoji `brand_icon`, karena kartu login butuh sesuatu di slot besar 52px-nya.
...
**Dua kartu paling kiri ("Harus siap hari ini"/"besok") pakai sumbu `order_date`, sisanya `created_at`.** Ini sengaja: pendapatan itu pertanyaan uang (kapan pesanan masuk), "besok bikin apa" itu pertanyaan dapur (kapan pesanan harus siap). Buat toko yang kerja per tanggal, dua angka itu bisa jauh beda. Statusnya juga beda: kartu produksi pakai `isProductionOrder()` (termasuk `pending`), kartu duit pakai `REVENUE_STATUSES` (gak termasuk `pending`). Kartunya `<button>` yang tembus ke tab Pesanan dengan tanggal itu kepilih dan filter status dibuka ke "Semua" (kalau tetap `pending`, tanggal yang semua pesanannya udah dikonfirmasi bakal keliatan kosong padahal justru penuh). Jangan kasih label yang mirip "Pesanan hari ini" yang udah ada, dua kartu berlabel mirip dengan angka beda lebih bikin bingung daripada gak ada kartunya.
...
- `OrderDetailModal` aksi: **Konfirmasi/Selesai/Batalkan** → `updateOrderStatus()`, yang merutekan status `cancelled` ke `cancel_order()` dan status lain ke update kolom biasa (lihat "Pembatalan & stok" di bawah); **Print Label** → `window.print()` (lihat catatan portal di bawah); **WA Customer** → `buildAdminOrderSummaryMessage()` (`shared/lib/whatsapp.js`) buka `wa.me/{customer_wa}` terisi otomatis.
- **`PrintLabel.jsx` pakai React Portal** (`createPortal`) ke `#printLabel`, `<div>` direct child `<body>` di `admin/index.html` (bukan di dalam `#root`) — CSS `@media print` di `admin.css` pakai selector `body > *:not(#printLabel)`. Kalau `#printLabel` dipindah ke dalam tree React normal, print CSS-nya rusak. `PrintLabel` selalu terima **array** (`orders`), modal detail ngirim `[order]`, biar satu label dan sekumpulan label gak mungkin beda tata letak cetaknya.
- **Print label batch** — tombol "Print N Label" di header tab nyetak SEMUA yang lagi ke-filter (status + tanggal), jumlahnya disebut di tombolnya biar gak ada yang gak sengaja ngirim ratusan halaman. Dua hal yang jangan dibalik: `#printLabel` harus `position: static`, **bukan** `fixed` (elemen fixed cuma kecetak di halaman pertama, jadi label kedua dst hilang tanpa jejak), dan `window.print()` dipanggil dari `useEffect` setelah render, bukan langsung di handler tombol (portal-nya belum ke-commit, yang kecetak halaman kosong).
...
Form yang upsert ke tabel `settings` (row `id=1`) lewat `saveSettings()` (`shared/lib/settings.js`): nama/ikon/logo brand, favicon, alamat & jam toko, link Maps, banner katalog, link Instagram/TikTok. Upload logo, favicon & banner lewat bucket `product-images`.

**Favicon** (`favicon_url`) di-upload di sini, bukan dengan mengganti file di `public/` — satu build dipakai banyak klien, sedangkan isi `public/` ikut ter-bundle sekali waktu build. `useFavicon()` (`shared/hooks/`) yang memasangnya saat runtime di ketiga app: link ikon statis dari `index.html` **dibuang dulu** (browser memilih ikon berdasarkan atribut `sizes`, jadi `favicon-32x32.png` yang dibiarkan bisa menang dari favicon custom yang tanpa `sizes`), baru dipasang `<link rel="icon">` + `<link rel="apple-touch-icon">` yang ditandai `data-ordi-favicon`. Kalau `favicon_url` kosong, hook-nya no-op dan link statis tetap dipakai.

**Brand header customer punya 2 slot gambar independen**, bukan satu "logo" — `logo_url` (logo ikon/mascot, gantiin emoji `brand_icon` di slot kecil) dan `logo_text_url` (logo teks/wordmark, gantiin tulisan `brand_name` di sebelahnya). Keduanya opsional dan independen: admin bisa pasang cuma salah satu, dua-duanya, atau gak sama sekali (fallback ke emoji+teks polos). Dirender di `OrderTypeStep.jsx` dan `CatalogStep.jsx` — tiap slot punya CSS class sizing sendiri (`brand-icon-logo` buat logo ikon, `ob-brand-name-logo`/`topbar-brand-name-logo` buat logo teks), jangan ketuker kalau nambah tempat baru yang nampilin brand header. `brand_name` (teks) tetap dipakai apa adanya di tempat non-visual (judul tab browser, pesan WA, dsb) — cuma tampilan header customer yang bisa diganti gambar.
...
**Guard sebelum bayar** — `App.jsx`'s `handleSubmitQris()` fetch kuota **ULANG** ke server sebelum buka QR, bukan pakai angka yang udah ada di layar, lalu cegat lewat `findCapacityBlocker()` (`shared/lib/capacity.js`). Ini bukan paranoia: di alur ini customer mindai dan **bayar DULU**, order baru ke-insert pas dia tekan konfirmasi. Kalau slotnya keburu diambil orang lain di sela itu, `place_order()` nolak `KUOTA_HABIS` setelah duitnya terlanjur ditransfer. Satu round trip jauh lebih murah dari itu. Ini pemanggil ketiga yang boleh ada tanpa ngelanggar pola "jangan reaktif" (lihat "Kenapa begini" soal ongkir): pemicunya tekan tombol, sekali jalan, bukan `useEffect` yang watch keranjang. `useCapacity().refetch()` balikin Map-nya (bukan cuma set state) khusus buat ini, dan kalau fetch-nya gagal dia balikin Map kosong biar gangguan jaringan gak berubah jadi customer yang gak bisa bayar sama sekali.

### Tanggal libur (`closed_dates`)

Tabel kecil berisi tanggal toko tutup, diatur dari **Admin > Pengaturan > Tanggal Libur**. Level **TOKO**, bukan per produk: yang dibutuhkan toko itu "Minggu saya libur", dan itu nutup semua produk sekaligus. Kuota per produk per tanggal (`capacity_overrides`) sengaja belum dibuat — belum ada yang butuh, ada jalan keluarnya (admin ubah `daily_capacity` produk itu sehari itu), dan bikin tabel tanpa UI cuma nambah beban.

**Ditegakkan `place_order()`** (error `TOKO_TUTUP:`), bukan cuma disembunyiin dari kalender. Tanpa itu nutup tanggal jadi kosmetik: tanggal bisa ditutup SETELAH customer milihnya.

`anon` sengaja gak dikasih policy SELECT. Customer lihat tanggal liburnya lewat `get_full_dates()` yang SECURITY DEFINER dan udah bawa alasannya, jadi SELECT langsung gak pernah dibutuhin. Kalau nanti kalender mau nampilin `note` ("Libur Lebaran"), tambahin kolomnya ke balikan `get_full_dates()`, **jangan** buka SELECT-nya.

GRANT-nya ditulis eksplisit, beda dari tabel lain di file itu yang ngandelin default privileges Supabase: tabel lain lahir bareng instance-nya, `closed_dates` lahir belakangan di instance yang udah jalan.

### Pembatalan & stok (`cancel_order()`)

`updateOrderStatus()` (`shared/lib/orders.js`) **merutekan status `cancelled` ke `cancel_order()`**, status lain tetap update kolom biasa. Ini yang bikin stok beneran balik — update dari browser gak bisa nyentuh `stock_qty`.

**Idempoten dan itu wajib**: tombol Batalkan bisa ketekan dua kali, dan tanpa syarat `status <> 'cancelled'` stoknya nambah dua kali. Cuma transisi pertama yang memulihkan.

Produk `stock_qty NULL` dilewati (emang gak pernah dikurangi), item tanpa `pid` juga. Batasan yang diterima sadar: produk yang diubah dari NULL jadi terlacak SETELAH pesanannya masuk bakal kelebihan stok saat dibatalkan. Nutupnya butuh nyatat persis apa yang dikurangi saat order dibuat, dan itu kolom baru buat kasus yang jarang.

Kuota harian gak kena masalah ini sama sekali, karena dihitung ulang dari tabel `orders` — lihat alasannya di atas.
...
- **Bias alamat pakai `viewbox` saja, TANPA `bounded=1`** — `bounded=1` memotong keras semua hasil di luar kotak. Kelihatan lebih "rapi", tapi artinya alamat yang benar dan cuma sedikit lewat radius hilang sama sekali dan customer buntu tanpa jalan keluar (gak ada cara buat bilang "iya saya tahu ini agak jauh"). Tanpa `bounded`, `viewbox` cuma jadi preferensi: yang dekat naik, yang jauh tetap muncul di bawah dengan label jaraknya. Yang berhak menolak alamat kejauhan itu cek ongkir (`SET_SHIPPING_UNAVAILABLE`) dan guard sebelum bayar, bukan kotak pencarian. Kalau nanti kepikiran nyalain `bounded=1` biar "lebih bersih", ini alasannya jangan.
...
- **Nambah env var baru di Cloudflare Settings gak otomatis masuk ke build yang lagi live** — Cloudflare Workers/Pages snapshot daftar env var pas build itu dijalankan; kalau var-nya ditambahin SETELAH build terakhir, build itu gak akan pernah punya var itu walau Settings-nya udah keupdate (bisa dicek dari log build → "Build variables", daftarnya statis per-build). Setelah nambah/ganti env var baru, wajib trigger build baru (**Retry build**, atau push commit baru) — ini yang bikin `VITE_GA_ID` sempat gak kesubstitusi (`%VITE_GA_ID%` masih literal di HTML live) padahal Settings-nya udah bener.
